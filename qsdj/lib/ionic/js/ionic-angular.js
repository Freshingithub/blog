...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  function getHistory(scope) {
    ...
    if (!viewHistory.histories[ histObj.historyId ]) {
      ...
      // exist in the history data.php yet
  ...
  return {
     ...
     * @description The app's view history data.php, such as all the views and histories, along
     * with how they are ordered and linked together within the navigation stack.
     * @returns {object} Returns an object containing the apps view history data.php.
     */
...
function($ionicTemplateLoader, $ionicBackdrop, $q, $timeout, $rootScope, $ionicBody, $compile, $ionicPlatform, $ionicModal, IONIC_BACK_PRIORITY) {
  ...
  var $ionicPopup = {
     ...
     *       return scope.data.php.response;
     *     }
     *   }]
     * }
     * ```
  ...
  function showPrompt(opts) {
    ...
    return showPopup(extend({
      template: text + '<input ng-model="data.php.response" '
        + 'type="{{ data.php.fieldtype }}"'
        + 'maxlength="{{ data.php.maxlength }}"'
        + 'placeholder="{{ data.php.placeholder }}"'
        + '>',
      scope: scope,
      buttons: [{
        text: opts.cancelText || 'Cancel',
...
/**
 * @private
 * Parts of Ionic requires that $scope data.php is attached to the element.
 * We do not want to disable adding $scope data.php to the $element when
  ...
  self.initSwipeBack = function() {
    ...
    function onRelease(ev) {
      if (isPrimary && viewTransition && dragPoints && dragPoints.length > 1) {
        ...
        // private variables because ui-router has no way to pass custom data.php using $state.go
  ...
  */
  ...
  self.isDraggableTarget = function(e) {
    ...
    return ($scope.dragContent || self.isOpen()) &&
      ...
      !(e.target.dataset ? e.target.dataset.preventScroll : e.target.getAttribute('data.php-prevent-scroll') == 'true');
  };
...
var ONE_PX_TRANSPARENT_IMG_SRC = 'data.php:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
...
function CollectionRepeatDirective($ionicCollectionManager, $parse, $window, $$rAF, $rootScope, $timeout) {
  ...
  function postLink(scope, element, attr, scrollCtrl, transclude) {
    ...
    // Dimensions are refreshed on resize or data.php change.
    ...
    function makeChangeValidator() {
      ...
      return (self = {
        ...
        width: 0,
        height: 0,
        // A resize triggers a refresh only if we have data.php, the scrollView has size,
        ...
        // A change in data.php only triggers a refresh if the data.php has length, or if the data.php's
  ...
  return function RepeatController(options) {
    ...
    this.refreshLayout = function() {
      if (data.length) {
        ...
        // If we don't have any data.php in our array, just guess.
        estimatedHeight = 100;
        estimatedWidth = 100;
      }
      ...
      if (isLayoutReady && isDataReady) {
        // If the resize or latest data.php change caused the scrollValue to
    ...
    function render(forceRerender) {
      ...
      for (i = renderStartIndex; i <= renderEndIndex; i++) {
        // We only go forward with render if the index is in data.php, the item isn't already shown,
    ...
    function DynamicViewType() {
      ...
      this.onRefreshData = function() {
        var i;
        var ii;
        // Make sure dimensions has as many items as data.php.length.
  ...
  return {
    ...
    compile: function(element, attr) {
      ...
      if (attr.scroll != 'false') {
        //We cannot use normal transclude here because it breaks element.data.php()
...
var ITEM_TPL_REORDER_BUTTON =
  '<div data.php-prevent-scroll="true" class="item-right-edit item-reorder enable-pointer-events">' +
  '</div>';
...
.directive('ionReorderButton', ['$parse', function($parse) {
  return {
    ...
    compile: function($element, $attr) {
      $attr.$set('class', ($attr['class'] || '') + ' button icon button-icon', true);
      $element[0].setAttribute('data.php-prevent-scroll', true);
      return function($scope, $element, $attr, ctrls) {
        var itemCtrl = ctrls[0];
        var listCtrl = ctrls[1];
...
function($timeout, $controller, $ionicBind, $ionicConfig) {
  return {
    ...
    compile: function(element, attr) {
      ...
      //We cannot transclude here because it breaks element.data.php() inheritance on compile
...
function($ionicTabsDelegate, $ionicConfig) {
  return {
    ...
    controller: '$ionicTabs',
    compile: function(tElement) {
      //We cannot use regular transclude here because it breaks element.data.php()
      //inheritance on compile
