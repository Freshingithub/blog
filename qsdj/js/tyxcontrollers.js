...
.controller('RouteCtrr',function($scope){
    ...
    .controller('OneKeyOderCtr',function($http,$scope,$rootScope,$ionicPopup,$document,$ionicLoading,$cordovaGeolocation,$location,$interval,$state){
        ...
        function createMap(){
            ...
            var aa= navigator.geolocation.getCurrentPosition(function(position){ 
                var lat = position.coords.latitude;
                var long = position.coords.longitude;
                //alert(lat+"--------"+long);
                $rootScope.point = new BMap.Point(long, lat);
                map.centerAndZoom($rootScope.point, 14);
                var marker = new BMap.Marker($rootScope.point);
                map.addOverlay(marker);
                //获取定位位置
                var geoc = new BMap.Geocoder();
                geoc.getLocation($rootScope.point, function(res) {
                    //$rootScope.location_city = res.addressComponents.city;
                    var addComp=res.addressComponents;
                    $rootScope.city=addComp.city;
                    //alert($rootScope.city);
                    $rootScope.genral=addComp.city + addComp.district + addComp.street + addComp.streetNumber;
                ...
                var options = {
                    ...
                    $cordovaGeolocation.getCurrentPosition(options).then(function(position) {    //h5地理定位
                        ...
                        //alert(lat+"--------"+long);
                        $rootScope.point = new BMap.Point(long, lat);
                        map.centerAndZoom($rootScope.point, 15);
                        //alert(lat+"----"+long);
                        var geoc = new BMap.Geocoder();
                        ...
                        convertor.translate(pointArr, 1, 5, function (data) {
                            if (data.status === 0) {
                                ...
                                geoc.getLocation(data.points[0], function (rs) {
                                    ...
                                    //$rootScope.location_city = rs.addressComponents.city;
                                     $rootScope.city=addComp.city;
                                     //alert($rootScope.city);
                                    $rootScope.genral=addComp.city + addComp.district + addComp.street + addComp.streetNumber;
                                    $ionicPopup.alert({title:addComp.province + ", " + addComp.city + ", " + addComp.district + ", " + addComp.street + ", " + addComp.streetNumber});
    ...
    .controller('OneKeyOderLaterCtr',function($scope,$location,$ionicPopup,$http,$rootScope,$document,$timeout,$filter,$state,$ionicLoading,$interval,$cordovaGeolocation){
        ...
        $scope.order = function() {
            ...
            if($rootScope.lon !== undefined && $rootScope.lat !== undefined) {
                var c = $rootScope.point;//new BMap.Point($rootScope.lon, $rootScope.lat);
...
//代叫
    .controller('DaiJiaoCtr',function($scope,$ionicActionSheet,$http,$ionicModal,$ionicPopup,$rootScope,$filter,$timeout,$state,$document,$stateParams,$ionicLoading,$cordovaContacts, $ionicPlatform,$location,$interval) {
        $scope.flag=true;
        var start_lon,start_lat;
        $scope.genral=function(){
            $scope.flag=false;
            $scope.eulav=$rootScope.genral;
            $scope.Taddress=$scope.eulav;
        }
        $scope.output=function(){
            $scope.flag=true;
            $rootScope.popup.close();
            $rootScope.popup.isPopup=false;
            $scope.myform.djfrom=$scope.Taddress;
            start_lon=$rootScope.lon;
            start_lat=$rootScope.lat;
        }
    ...
    .controller('pagCtr',function($scope,$http,$rootScope,$timeout,$ionicLoading,FloatStr,$stateParams,$state,$ionicPopup){
        ...
        $scope.immediatePay=function(pageForm){
            ...
            if(pageForm.valid){
                ...
                $scope.order_number=out_trade_no;
                //价格
                ...
                if (pageForm.pay==$scope.wechatPay){
                    ...
                    pay_type="wechatPay";
                    $scope.pay_type="wechatPay";
                    var bodtxt = "订单支付";
                    var url = "http://weixin.qq.com"; //异步同步接口
                    console.log(out_trade_no+url+bodtxt+price);
                    window.plugins.Pgwxpay.wxpay(out_trade_no, url, bodtxt, price * 100, function (success) {
                        //alert(success);
